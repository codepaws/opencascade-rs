...
// Handle_Poly_Triangulation is already defined by Poly_Triangulation.hxx via
// DEFINE_STANDARD_HANDLE(). On MSVC it expands to a class (not a typedef),
...
// On MSVC, DEFINE_STANDARD_HANDLE expands Handle_Poly_Triangulation to a class
// (not a typedef), so the template above won't deduce T=Poly_Triangulation for it.
// Provide an explicit overload so the CXX-generated trampoline compiles.
...
inline std::unique_ptr<Handle_Poly_Triangulation> BRep_Tool_Triangulation(const TopoDS_Face &face,
  ...
  // Construct Handle_Poly_Triangulation (may be a class on MSVC) from the raw handle.
...
inline void compute_normals(const TopoDS_Face &face, const Handle_Poly_Triangulation &triangulation) {
